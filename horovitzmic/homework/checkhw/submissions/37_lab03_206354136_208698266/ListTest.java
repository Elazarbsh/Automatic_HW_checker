...
public abstract class ListTest<T> {
	...
	public void testHasNextAndPrev()
	{
		...
		//two nodes in list, curser on the second
		dList.insert(newInstance2);
		assertTrue(dList.hasPrev());
		assertFalse(dList.hasNext());

		//curser moves to the first node
		dList.goToBeginning();
		assertTrue(dList.hasNext());
		assertFalse(dList.hasPrev());

		//curser moves to second node
		dList.getNext();
		assertTrue(dList.hasPrev());
		assertFalse(dList.hasNext());

		//adding third node after the second, and cursor moves to the third
		dList.insert(newInstance3);
		assertFalse(dList.hasNext());
		assertTrue(dList.hasPrev());

		// curser moves twice (to the first node)
		dList.getPrev();
		dList.getPrev();
		assertFalse(dList.hasPrev());
		assertTrue(dList.hasNext());
