...
public abstract class ListTest<T> {
	...
	public void testHasNextAndPrev() {
		...
		dList.insert(newInstance3);
		dList.insert(newInstance1);
		// checking in the end of the list
		assertFalse(dList.hasNext());
		assertTrue(dList.hasPrev());
		dList.remove();
		dList.goToEnd();
		...
		assertFalse(dList.hasPrev());
		assertTrue(dList.hasNext());
		// checking with one element in the list
		dList.remove();
		assertFalse(dList.hasPrev());
		assertFalse(dList.hasNext());
		...
		assertTrue(dList.hasNext());
		assertTrue(dList.hasPrev());
		// checking empty list (with remove)
		dList.remove();
		dList.remove();
		dList.remove();
		assertFalse(dList.hasNext());
		assertFalse(dList.hasPrev());
		// checking empty list (with clear)
		dList.insert(newInstance1);
		dList.clear();
		assertFalse(dList.hasNext());
		assertFalse(dList.hasPrev());
