...
public abstract class ListTest<T> {
	...
	@Test
	public void testHasNextAndPrev() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
		List<T> l = dListFactory.getDList();
		assertFalse(l.hasNext());
		assertFalse(l.hasPrev());
		T ourInstance1 = getParameterInstance();
		T ourInstance2 = getParameterInstance();
		l.insert(ourInstance1);
		assertFalse(l.hasNext());
		assertFalse(l.hasPrev());
		l.insert(ourInstance2);
		assertFalse(l.hasNext());
		assertTrue(l.hasPrev());
	}
